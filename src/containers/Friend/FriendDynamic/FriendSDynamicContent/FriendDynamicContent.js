...
export default class FriendDynamicContent extends Component {
  ...
  componentWillMount(){
    this.data=Data.user;
    this.PraiseStyle={color: "red"};
    this.noPraiseStyle={color: "#616162"};
  }
  render() {
    return (
      <div>
        {this.data.map((item, index) => {
          let {username,contentTitle}= item;
          return <div key={index} className="each-content">
            ...
            <footer className="friend-dynamic-number">
              <span className="iconfont icon-zan" style={this.state.praise ? this.PraiseStyle : this.noPraiseStyle}
