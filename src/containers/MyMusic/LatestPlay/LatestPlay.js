    ...
    return (
      <div>
        ...
        <div className="content">
          ...
          <ul className='songs'>
            {this.props.record.weekData.slice(0, 2).map((item, index) => {
              item.isSelscted = false;
              return (<li className={this.state.isTurn ? ' active' : ''}>
                <Link to={'/'}>
                  <div className='song-name'>
                    {item.isSelected?
                      /*未选中*/<i
                        className={`choice-none iconfont icon-weixuanzhongyuanquan${!this.state.isTurn ? ' active' : ''}`}></i>

                      /*选中*/ : <i className="choiced iconfont icon-2xuanzhong"></i>}
                    <p>彩虹</p>
                    <span>周杰伦 - 主打</span>
                  </div>
